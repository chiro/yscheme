...
(defmethod call-with-input-file ((file scm-string) (proc procedure))
...
(defmethod call-with-output-file ((file scm-string) (proc procedure))
...
(defgeneric with-input-from-file (obj1 obj2))
(defmethod with-input-from-file ((file scm-string) (thunk procedure))
  (with-open-file (in file :if-does-not-exist :error)
    (let ((*current-input-port* (new 'scm-port :file file :direction :input :strm in)))
      (scm-apply thunk nil))))

(defgeneric with-output-to-file (obj1 obj2))
(defmethod with-output-to-file ((file scm-string) (thunk procedure))
  (with-open-file (out file :direction :output :if-exists :error)
    (let ((*current-output-port*
           (new 'scm-port :file file :direction :output :strm out)))
      (scm-apply thunk nil))))

(defgeneric open-input-file (obj))
(defmethod open-input-file ((file scm-string))
  (new 'scm-character-port
       :file file
       :direction :input
       (open file :if-does-not-exists :error)))

(defgeneric open-binary-input-file (obj))
(defmethod open-binary-input-file ((file scm-string))
  (new 'scm-binary-port
       :file file
       :direction :input
       (open file :element-type 'unsigned-byte :if-does-not-exists :error)))

(defgeneric open-output-file (obj))
(defmethod open-output-file ((file scm-string))
  (new 'scm-character-port
       :file file
       :direction :output
       (open file :direction :output :if-does-not-exists :error)))

(defgeneric open-binary-output-file (obj))
(defmethod open-binary-output-file ((file scm-string))
  (new 'scm-binary-port
       :file file
       :direction :output
...
(defgeneric close-port (obj))
(defmethod close-port ((port scm-port))
  (and (close (strm port)) +true+))

(defgeneric close-input-port (obj))
(defmethod close-input-port ((port scm-port))
  (if (eql (direction port) :input)
      (and (close (strm port)) +true+)
      +false+))

(defgeneric close-output-port (obj))
(defmethod close-output-port ((port scm-port))
  (if (eql (direction port) :output)
      (and (close (strm port)) +true+)
      +false+))

;(defgeneric open-output-string ())

;(defgeneric get-output-string (obj))

;(defgeneric open-output-bytevector ())

;(defgeneric get-output-bytevector (obj))
...
(define-predicate eof-object? ((obj scm-eof)) +false+ +true+)

;(define-predicate char-ready? ((obj scm-character-port))
...
;(define-predicate u8-ready? ((obj scm-binary-port))
...
(defgeneric scm-write-bytevector (obj1 obj2 obj3 &optional obj4))
(defmethod scm-write-bytevector
    ((bytevec scm-bytevector) (start scm-integer) (end scm-integer)
     &optional (port *current-input-port*))
...
(defgeneric flush-output-port (&optional port))
(defmethod flush-output-port (&optional (port *current-input-port*))
  +true+)